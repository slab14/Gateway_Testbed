...
setup_ip_routes() {
    local interface=$(find_interface_for_ip $CLIENT_IP || find_interface_for_ip $SERVER_IP)
    ...
    sudo ip route add $net/16 dev $interface || sudo ip route change $net/16 dev $interface
}
